...
- [ ] Use only the `/api/rest/mva/out/cloud/*` paths documented by this Skill.
...
- [ ] Use `scripts/make_from_local_media.py` from a trusted runtime with access to explicitly selected local files.
- [ ] Keep API keys out of browser/mobile clients, prompts, command output, and repositories.
- [ ] Send basename, byte size, MIME type, and SHA-256 to `/upload/init`; do not send local bytes through the gateway.
- [ ] Use the COS SDK high-level transfer API with the exact returned Bucket, Region, object key, part size, and required headers.
- [ ] Keep temporary COS credentials in memory, use them for one object only, and discard them immediately after transfer.
- [ ] Send only `upload_id` to `/upload/complete` and require one descriptor with a non-empty `url` before `/make`.
...
- [ ] Retry ambiguous `/upload/complete` responses with the same `upload_id`; initialize a new session after `409105`.
- [ ] Record and clean up unused objects after ambiguous direct-upload retries.
